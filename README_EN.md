...
### 📋️ Function Signs
Function sign monuments provided by the world, containing the following three function signs.
...
- Build a portal frame using the themed block of the respective world and activate it with a fire source to travel to different dimensions.
- Unlike Nether Portals, regardless of the coordinates, a Void Portal will search for the nearest portal within a 256-block radius centered on the world spawn of the target dimension. If none is found, a portal structure will be generated near the world spawn.
...
> (e.g., In the "Persisting Worlds" world, if the Sustainer is transported (deleting and respawning the entity), its data will be corrupted, allowing it to pass through portals and causing the border energy of its respective dimension to continuously drain)  
...
**A:** Check if you are in Peaceful mode. If the world defaults to Peaceful mode, please provide the world name for a fix.  
...
**Q: After building a multi-dimensional mob farm using the Void Portal mechanics in the "Persisting Worlds" world, I'm experiencing severe TPS lag/delay?**
